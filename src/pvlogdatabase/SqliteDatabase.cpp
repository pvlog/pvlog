...
void SqliteDatabase::prepare(const std::string& statment)
{
	...
	if (sqlite3_prepare_v2(db, statment.c_str(), statment.size() + 1, &stmt, 0) != SQLITE_OK) PVLOG_EXCEPT
	        ( sqlite3_errmsg(db));
}

void SqliteDatabase::exec(std::vector<Value>& values)
{
	if (sqlite3_reset(stmt) != SQLITE_OK) PVLOG_EXCEPT("Failed to reset statement!");

	if (values.size() != static_cast<size_t> (sqlite3_bind_parameter_count(stmt))) {
		values.clear();
		PVLOG_EXCEPT("Parameter count mismatch.");
	}
...
{
	if (static_cast<size_t> (index) >= values.size()) PVLOG_EXCEPT("Invalid index!");
                          ...
                          const std::string &)
{
	...
	if (sqlite3_open_v2(filename.c_str(), &db, SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE, NULL)
	        != SQLITE_OK) {
		PVLOG_EXCEPT( sqlite3_errmsg(db));
	}
}
...
void SqliteDatabase::close()
{
	...
	if (sqlite3_close(db) != SQLITE_OK) PVLOG_EXCEPT( sqlite3_errmsg(db));
