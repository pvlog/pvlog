...
void DataStorage::storeAc(const Ac & ac)
{
    query->beginTransaction();

    for (int i = 0; i < ac.lineCount; i++) {
        struct tm * timeInfo = localtime(&ac.time);
        char time[20];
        strftime(time, 20, "%Y-%m-%d %H:%M:%s", timeInfo);

        query->prepare("INSERT INTO ac_values, Values(:line, :id, :date, :power, :voltage, :current);");
        query->bindValueAdd(i);
        query->bindValueAdd(ac.id);
        query->bindValueAdd(time);
        query->bindValueAdd(ac.power[i]);
        query->bindValueAdd(ac.voltage[i]);
        query->bindValueAdd(ac.current[i]);
        query->exec();
    }
...
void DataStorage::storeDc(const Dc & dc)
{
    query->beginTransaction();

    for (int i = 0; i < dc.trackerCount; i++) {
        ...
        query->bindValueAdd(i);
        query->bindValueAdd(dc.id);
        query->bindValueAdd(time);
        query->bindValueAdd(dc.power[i]);
        query->bindValueAdd(dc.voltage[i]);
...
int DataStorage::numInverter(int id)
{
    if (id < 0) {
        query->exec("SELECT count(*) AS inverter_num from string_inverter;");
    } else {
...
void DataStorage::getInverters(std::vector< std::pair<int64_t, std::string> > & inverters, int plantId)
{
    if (plantId < 0) {
